...
# def process_segments(timestamps: np.ndarray, values: np.ndarray, delta_time: np.int64, max_timestamps: int) -> List[List[float]]:
#     segments = []
#     segment = []
#     difference = np.int64(timestamps[1:] - timestamps[:-1])

#     # Process each value
#     for diff, value in zip(difference, values[1:]):
#         coeff = diff // delta_time
#         if int(coeff) != coeff or coeff > max_timestamps:
#             if segment:  # Check if the segment has any data
#                 segments.append(segment)
#             segment = [value]
#         elif np.abs(diff / delta_time) - 1 < 1e-1:
#             segment.append(value)
#         elif int(coeff) == coeff or coeff <= max_timestamps:
#             values_to_add = [None] * (coeff - 1) + [value]
#             segment.extend(values_to_add)
...
#     differences_int = differences.astype(int)
    
#     for diff, value in zip(differences_int, values[1:]):
#         quotient = diff // delta_time
#         remainder = diff % delta_time
        
#         if quotient > max_timestamps or remainder != 0:
